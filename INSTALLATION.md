# Ninj-OS Proxie Edge Fabric v7.3.12 installation index

v7.3.12 is one universal package for Pterodactyl, standalone Linux, Docker on Linux, and Windows through WSL2. The same proxy can mix native `online-mode=true` routes with proxy-authenticated `online-mode=false` routes.
