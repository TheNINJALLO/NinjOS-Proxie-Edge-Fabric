...
3. Build Endstone Companion v3.7.0 against the exact Endstone API installed on the backend.
...
profile, and choose `operator`. v7.3.13 saves the selection immediately. Fully
disconnect and rejoin so the new signed identity grant applies the role and
refreshes Endstone's command list.

On join, Companion v3.7.0 retrieves the one-use grant asynchronously, briefly
