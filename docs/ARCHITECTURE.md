...
Each enabled Full Proxy backend has its own public UDP listener. Current v7.3.11 switching uses a controlled Bedrock transfer to another Ninj-OS listener. A retained-session downstream hot-swap remains a later hardening target and is not represented as complete.
