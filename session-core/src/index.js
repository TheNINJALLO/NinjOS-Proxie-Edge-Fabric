...
function handleProxyCommand (backend, player, packet, descriptor) {
  ...
  if (name === 'server') {
    ...
    player.queue('text', { type: 'system', needs_translation: false, source_name: '', xuid: '', platform_chat_id: '', filtered_message: '', message: '§bNinj-OS Proxie v7.3.12 · Full Proxy Session Core' })
  }
  return true
}
...
function makeRelay (backend) {
  ...
  relay.on('connect', (player) => {
    ...
    player.on('clientbound_raw', ({ packetId, packet }) => {
      protocolWeave.observeLosslessPassthrough(
        player.__ninjosProtocolPack,
        backend.id,
        'clientbound',
        packet,
        packetId
